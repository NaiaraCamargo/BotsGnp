...
from funcoespncp import logs, enviar_mensagem, obter_pastas_download
...
def processar_edital(edital, ids_usuarios, error_timeout, palavras_destacadas, filtrosBase, plataforma, lista_erros_api):
    ...
    try:    
        ...
        if not isinstance(compra, dict):
            print(f"[EDITAL IGNORADO - COMPRA API INDISPONÍVEL] {link}")
            logs.error(f"[EDITAL IGNORADO - COMPRA API INDISPONÍVEL] {link}")
            return None, error_timeout
        ...
        if edital["Uf"].upper() == "RS" or edital["Uf"].upper() == "SC":
